...
private fun part1(input: String, v1: Int, v2: Int): Pair<String, Int> {
    ...
    val instructions = mutableMapOf<String, BotInstruction>()
    val outputs = mutableMapOf<String, Int>()

    val valueRegex = """value (\d+) goes to bot (\d+)""".toRegex()
    val givesRegex = """bot (\d+) gives low to (bot|output) (\d+) and high to (bot|output) (\d+)""".toRegex()

    input.lines().filter { valueRegex.matches(it) }
        .forEach {
            val (value, bot) = valueRegex.matchEntire(it)!!.destructured
            val current = bots.getOrPut(bot) { Bot(bot, mutableListOf()) }
            current.chips.apply { add(value.toInt()) }.sort()
        }
    input.lines().filter { givesRegex.matches(it) }
        .forEach {
            val (b1, lowType, lowNum, highType, highNum) = givesRegex.matchEntire(it)!!.destructured
            instructions[b1] = BotInstruction(b1, lowType, lowNum, highType, highNum)
        }

    while (instructions.isNotEmpty()) {
        val (key, instruction) = instructions.entries.first { bots[it.key]?.chips?.size == 2 }
        instructions -= key

        val bot = bots[key]!!
        if (instruction.lowTarget == "bot") {
            val low = bots.getOrPut(instruction.lowName) { Bot(instruction.lowName, mutableListOf()) }
            low.chips.apply { add(bot.chips[0]) }.sort()
        } else {
            outputs[instruction.lowName] = bot.chips[0]
        }

        if (instruction.highTarget == "bot") {
            val high = bots.getOrPut(instruction.highName) { Bot(instruction.highName, mutableListOf()) }
            high.chips.apply { add(bot.chips[1]) }.sort()
        } else {
            outputs[instruction.highName] = bot.chips[1]
        }
    }

    val result = bots.values.first { it.chips == listOf(v1, v2) }

    return result.name to outputs["0"]!! * outputs["1"]!! * outputs["2"]!!
}

private data class BotInstruction(val name: String, val lowTarget: String, val lowName: String, val highTarget: String, val highName: String)

private data class Bot(val name: String, val chips: MutableList<Int>)

private const val testInput1 = """value 5 goes to bot 2
...
private const val input = """value 23 goes to bot 208
