...
private fun part1(input: String) = input.lines().count {
    val words = it.split(' ')
    words.size == words.distinct().size
}

private fun part2(input: String) = input.lines().count { line ->
    val words = line.split(' ').map { it.toSet() }
    words.size == words.distinct().size
}

private const val testInput1 = """aa bb cc dd ee
aa bb cc dd aa
aa bb cc dd aaa"""

private const val input = """una bokpr ftz ryw nau yknf fguaczl anu
